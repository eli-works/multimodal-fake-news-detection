...
CFG = SimpleNamespace(
    ...
    dataset_root="weibo",
    processed_dir="weibo",
    train_csv="train_weibo.csv",
    val_csv="val_weibo.csv",
    test_csv="test_weibo.csv",
    ...
    use_shared_bridge=True,
    use_bicrossattn=True,
    ablation_name="transformer_only",
)
...
def create_run_dir(save_root: str) -> Path:
    ...
    run_dir = Path(save_root) / f"run_weibo_transformer_only_{ts}"
    ...
    if need_prepare:
        if not cfg.auto_prepare_data:
            raise FileNotFoundError(
                ...
                "Run `python prepare_weibo_data.py --dataset-root weibo --output-dir weibo` first."
            )
        logging.info("Preparing Weibo CSV files ...")
        from prepare_weibo_data import PreprocessConfig, run_preprocess
        ...
        logging.info("Weibo data preparation complete.")
...
class LSTMTransformerEncoder(nn.Module):
    def __init__(self, vocab_size: int, cfg) -> None:
        super().__init__()
        self.use_lstm = bool(getattr(cfg, "use_lstm_encoder", True))
        self.use_transformer = bool(getattr(cfg, "use_transformer_encoder", True))
        ...
        if self.use_lstm:
            ...
            self.lstm = None
            self.proj = nn.Linear(cfg.embed_dim, cfg.model_dim)

        self.pos = SinusoidalPositionalEncoding(cfg.model_dim, cfg.max_len)
        if self.use_transformer and int(cfg.transformer_layers) > 0:
...
class SharedResidualBridge(nn.Module):
    ...
    def _shared_residual(self, x: torch.Tensor) -> torch.Tensor:
        # 残差分支：Res(x)=dropout(W2*tanh(W1*x))
    ...
    def forward(self, text_vec: torch.Tensor, image_vec: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 共享语义：x_share=tanh(dropout(x)+alpha*Res(x))
...
class MultiModalModel(nn.Module):
    def __init__(self, vocab_size: int, cfg) -> None:
        super().__init__()
        self.text_encoder = LSTMTransformerEncoder(vocab_size, cfg)
        self.image_encoder = ImageEncoder(cfg)

        self.use_shared_bridge = bool(getattr(cfg, "use_shared_bridge", True))
        self.use_bicrossattn = bool(getattr(cfg, "use_bicrossattn", True))

        if self.use_shared_bridge:
            self.shared_bridge = SharedResidualBridge(dim=cfg.model_dim, dropout=cfg.dropout)
        else:
            self.shared_bridge = None

        if self.use_bicrossattn:
            self.cross_fusion = LightweightBiCrossAttentionFusion(
                dim=cfg.model_dim,
                heads=cfg.heads,
                num_layers=cfg.cross_attn_layers,
                image_tokens=cfg.cross_attn_image_tokens,
                dropout=cfg.dropout,
                ffn_ratio=cfg.cross_attn_ffn_ratio,
            )
        else:
    ...
    def forward(self, texts: torch.Tensor, text_mask: torch.Tensor, images: torch.Tensor) -> torch.Tensor:
        text_seq = self.text_encoder(texts, text_mask)
        image_vec = self.image_encoder(images)

        mask = text_mask.unsqueeze(-1).float()
        lengths = mask.sum(dim=1).clamp(min=1.0)
        text_vec = (text_seq * mask).sum(dim=1) / lengths

        if self.shared_bridge is not None:
            text_share, image_share = self.shared_bridge(text_vec, image_vec)
        else:
            text_share, image_share = text_vec, image_vec

        if self.cross_fusion is not None:
            text_tokens = text_seq + text_share.unsqueeze(1)
            text_fused, image_fused = self.cross_fusion(text_tokens, text_mask, image_share)
        else:
            # A2: replace bi-cross attention with simple concatenation fusion inputs.
            text_fused, image_fused = text_share, image_share
...
def main() -> None:
    ...
    logging.info("Ablation: %s", cfg.ablation_name)
    ...
    logging.info("Label convention: 0=nonrumor, 1=rumor.")
    logging.info(
